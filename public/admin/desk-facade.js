...
function createDeskFacade() {
  ...
  const facade = {
    ...
    async getPage(idOrOpts) {
      const id = typeof idOrOpts === "string" ? idOrOpts : idOrOpts && typeof idOrOpts === "object" ? idOrOpts.id : "";
      return loadPage(id);
    },
    async createPage({ id, path, title, description, open } = {}) {
      ...
      return {
        ...
        open: shouldOpen,
        navigated: false,
        message: shouldOpen ? `Created \u201C${data.id}\u201D as a local draft. Open ${editorPath} next.` : `Created \u201C${data.id}\u201D as a local draft. Call open_page to edit layout.`
      };
    },
    openPage({ id, force } = {}) {
      const pageId = String(id || "").trim();
