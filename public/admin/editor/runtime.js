...
function bootEditor() {
  ...
  const pageSwitcherBtn = document.getElementById("page-switcher-btn");
  ...
  const pageSwitcherFilter = document.getElementById("page-switcher-filter");
  ...
  function renderPageSwitcherList(pages, draftIds, query = "") {
    ...
    pageSwitcherList.innerHTML = filtered.map((p) => {
      const current = p.id === s.boot.id;
      const draft = draftIds.has(p.id);
      const title = p.title && p.title !== p.id ? `<span class="page-switcher-item-title">${escapeHtml(p.title)}</span>` : "";
  ...
  async function openImagePickerForPath(path) {
  ...
  window.addEventListener("keydown", (e) => {
    ...
    if (!mod && e.key === "Enter" && !e.shiftKey && !e.altKey) {
      const t = e.target;
      if (t instanceof HTMLInputElement && s.selectedPath && t.id === pathToFieldId(s.selectedPath) && !s.inlineEditPath) {
        e.preventDefault();
    ...
    if (data.type === "addListItem") {
      ...
      void (async () => {
        try {
          ...
          const index = result && typeof result === "object" && "index" in result ? Number(result.index) : NaN;
          if (!Number.isFinite(index)) return;
  ...
  function syncWebMcpChip() {
    ...
    if (info.ready) {
      ...
      chip.textContent = `WebMCP on \xB7 ${info.tools} tools`;
      ...
      chip.title = (info.errors || []).join(" \xB7 ") || "No modelContext API in this browser \u2014 enable WebMCP in Chrome/Cursor to drive the editor.";
    }
  }
      ...
      if (addList) {
        ...
        void (async () => {
          try {
            ...
            const itemIndex = result && typeof result === "object" && "index" in result ? Number(result.index) : NaN;
