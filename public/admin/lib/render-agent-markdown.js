...
function isSafeImageUrl(url) {
  try {
    ...
    return u.hostname === "res.cloudinary.com" || /\.(png|jpe?g|gif|webp|svg)(\?|$)/i.test(u.pathname);
  } catch {
    return false;
  }
}
function figureHtml(url, alt = "") {
  ...
  const caption = alt && String(alt).trim() ? `<figcaption>${escapeAttr(alt.trim())}</figcaption>` : "";
