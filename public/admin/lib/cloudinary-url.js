...
function deliveryThumbUrl(secureUrl, size = 320, publicId = "", mode = "fill") {
  const w = Math.max(64, Math.round(size));
  const transform = mode === "limit" ? `c_limit,w_${w},f_auto,q_auto` : `c_fill,g_auto,w_${w},h_${w},q_auto,f_auto,dpr_auto`;
