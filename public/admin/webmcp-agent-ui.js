...
function summarizeProposedArgs(tool, args) {
  ...
  if (tool === "set_field") {
    ...
    const raw2 = typeof args?.value === "string" ? args.value : JSON.stringify(args?.value);
    const value = raw2 && raw2.length > 60 ? `${raw2.slice(0, 57)}\u2026` : raw2 || "";
  ...
  if (tool === "patch_section") {
    ...
    return `section ${args?.index ?? "?"} \xB7 ${keys.join(", ") || "patch"}`;
  }
  const raw = JSON.stringify(args || {});
  return raw.length > 80 ? `${raw.slice(0, 77)}\u2026` : raw;
}
function appendActionCard(log, proposal, { execute } = {}) {
  const items = (Array.isArray(proposal?.items) ? proposal.items : []).filter((item) => item && PROPOSABLE_TOOLS.has(item.tool) && item.args && typeof item.args === "object").slice(0, 12).map((item) => ({
    tool: item.tool,
    args: item.args,
  ...
  wrap.append(head);
  const hintText = proposal?.hint || (items.some((i) => i.tool === "save_to_cms") ? "Review, then Apply to save \u2014 or Dismiss." : "Apply to run these changes on the page, or Dismiss.");
  ...
  for (const item of items) {
    ...
    const applyLabel = items.length === 1 && item.tool === "save_to_cms" ? "Save" : items.length === 1 && item.tool === "publish_changes" ? "Publish" : items.length === 1 && item.tool === "discard_changes" ? "Discard" : "Apply";
...
function appendImageResults(log, result) {
  ...
  const target = imageTargetFor(api);
  const wrap = el("div", { className: "tb-agent-msg tool tb-agent-gallery-wrap" });
  const visionBits = [];
  if (result?.metadata?.used && result.metadata?.terms?.length) {
    visionBits.push(`tags/title \xB7 ${result.metadata.terms.slice(0, 4).join(" ")}`);
  }
  if (result?.vision?.used) {
    visionBits.push(`vision \xB7 ${result.vision.candidates || "?"} thumbs`);
  ...
  for (const asset of assets.slice(0, 12)) {
    ...
    const label = el("span", {
      ...
      text: [
        typeof asset.visionScore === "number" ? `${Math.round(asset.visionScore * 100)}%` : typeof asset.metadataScore === "number" && asset.metadataScore > 0 ? `\u2605${asset.metadataScore}` : "",
        asset.orientation || "",
    ...
    useBtn.addEventListener("click", async (e) => {
      ...
      useBtn.textContent = "\u2026";
      try {
        const alt = asset.description || asset.title || asset.visionReason || asset.filename || asset.publicId || "Selected image";
