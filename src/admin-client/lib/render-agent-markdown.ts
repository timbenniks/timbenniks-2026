...
function promoteBareImageUrls(text: unknown): string {
...
export async function renderAgentMarkdown(markdown: unknown): Promise<string> {
...
export function bindAgentMarkdownMedia(root: ParentNode): void {
...
/** Bubbles carry the sequence of the last render so stale streams can't win. */
type MarkdownBubble = HTMLElement & { __mdSeq?: number };

/**
 * Update an assistant bubble's markdown HTML.
 * Concurrent calls are sequenced so only the latest text wins.
 */
export async function setBubbleMarkdown(bubble: MarkdownBubble, text: unknown): Promise<void> {
