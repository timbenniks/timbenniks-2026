import type { SectionKind } from '../lib/content.js';
...
export function fieldLabel(def: FieldDef, sectionIndex: number): string {
  if (def.label) return def.label;
  return humanizePath(`sections.${sectionIndex}.${def.key}`, sectionIndex).label;
}

export function editableLeafPaths(value: unknown, base = ''): string[] {
  const out: string[] = [];
...
export function pathToFieldId(path: string): string {
  return `field-${path.replace(/[^a-zA-Z0-9_-]/g, '_')}`;
}

export function pathCoveredByLists(
  path: string,
  sectionIndex: number,
  kind: SectionKind,
): boolean {
