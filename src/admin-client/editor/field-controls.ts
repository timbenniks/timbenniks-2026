...
import type { PageSection } from '../lib/content.js';
import { CANONICAL_TAG_OPTIONS, PLAYLIST_OPTIONS } from './catalog.js';
import type { FieldDef, FieldOption } from './catalog.js';
import { openMediaPickerModal } from './media-picker.js';
import type { CloudinaryInsertAsset } from './media-picker.js';
import { fieldLabel, humanizePath, pathToFieldId } from './paths.js';
import type { EditorSession, FieldInput } from './session.js';

type FieldControlsApi = Pick<
  EditorSession,
  | 'coerceFieldValue'
  | 'normalizeOption'
  | 'resolveFieldOptions'
...
export function createFieldControls(s: EditorSession): FieldControlsApi {
  function coerceFieldValue(def: FieldDef, raw: unknown): unknown {
  ...
  function normalizeOption(opt: unknown): FieldOption {
    if (opt && typeof opt === 'object' && 'value' in opt) {
      const record = opt as { value: unknown; label?: unknown };
      return { value: String(record.value), label: String(record.label ?? record.value) };
    }
    return { value: String(opt), label: String(opt) };
  }

  function resolveFieldOptions(def: FieldDef): FieldOption[] {
  ...
  function deleteByPath(obj: unknown, path: string) {
    const parts = path.split('.');
    let cur = obj as Record<string, unknown> | null | undefined;
    for (let i = 0; i < parts.length - 1; i++) {
      if (cur == null || typeof cur !== 'object') return;
      cur = cur[parts[i] as string] as Record<string, unknown> | null | undefined;
    }
    if (cur && typeof cur === 'object') delete cur[parts[parts.length - 1] as string];
  }
  ...
  function shortFieldPath(path: string): string {
    const m = String(path).match(/^sections\.\d+\.(.+)$/);
    return m ? (m[1] as string) : path;
  }

  function clearCardGridFiltersForSource(section: PageSection | undefined): boolean {
  ...
  function clearSourceDependentFilters(section: PageSection | undefined): boolean {
  ...
  function highlightFieldInForm(path: string | null) {
  ...
  function focusFieldInForm(path: string | null) {
    if (!path) return;
    const input = document.getElementById(pathToFieldId(path));
    if (!input) return;
  ...
  function bindLiveInput(input: FieldInput, path: string, kind?: string) {
  ...
  function applyCloudinaryAsset(path: string, asset: CloudinaryInsertAsset | null | undefined) {
    if (!asset) return;
    ...
    if (def.type === 'multi-select') {
      ...
      options.forEach((opt) => {
        ...
        input.addEventListener('change', () => {
          s.checkpoint();
          const next = [...group.querySelectorAll<HTMLInputElement>('input:checked')].map(
            (el) => el.value,
          );
