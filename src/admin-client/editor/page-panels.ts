...
import type { ChangesStatus, EditorSession, PageHistoryData } from './session.js';

type PagePanelsApi = Pick<
  EditorSession,
  'refreshInfoPane' | 'syncInfoEditStatus' | 'refreshHistoryPanel' | 'fetchChangesStatus'
>;
...
export function createPagePanels(s: EditorSession): PagePanelsApi {
  function formatCommitDate(iso: string | null | undefined): string {
    if (!iso) return '—';
    try {
  ...
  function editStatusLabel(): string {
  ...
  async function fetchChangesStatus(force = false): Promise<ChangesStatus> {
    if (!force && s.changesCache) return s.changesCache;
    try {
      const baseline = await apiFetch<Omit<ChangesStatus, 'draftIds' | 'hasThisDraft' | 'aheadBy'>>(
        '/api/admin/changes',
  ...
  async function fetchHistory(force = false): Promise<PageHistoryData> {
    if (!force && s.historyCache) return s.historyCache;
    try {
      s.historyCache = await apiFetch<PageHistoryData>(
